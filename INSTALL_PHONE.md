# Install FinanceFlow APK on your phone (v1.2.3)

## File to use
- Desktop: `FinanceFlow-v1.2.3.apk`
...
1. Copy `FinanceFlow-v1.2.3.apk` to the phone (USB, Google Drive, WhatsApp to yourself, or email).
...
5. Confirm the app shows **1.2.3** under Settings → Check for updates (or About).
...
1. Host `FinanceFlow-v1.2.3.apk` at the URL in `frontend/public/app-update.json` → `apkUrl` (Vercel `/releases/` or GitHub Release).
2. Deploy web + API so `GET /app/update` reports `versionCode: 6`.
